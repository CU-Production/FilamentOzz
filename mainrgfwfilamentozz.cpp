...
int main()
{
    ...
    RGFW_window* window = RGFW_createWindow("RGFW + Filament + ozz", RGFW_RECT(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT), RGFW_CENTER);
...
#pragma region ozz init
    ...
    else
    {
        ...
        for (int i = 0; i < (int)num_vertices; i++) {
            ...
            v->joint_weights[0] = std::fmax(jw0, 0.0f);
            v->joint_weights[1] = std::fmax(jw1, 0.0f);
            v->joint_weights[2] = std::fmax(jw2, 0.0f);
            v->joint_weights[3] = std::fmax(jw3, 0.0f);
        }
...
#pragma endregion ozz init
    ...
    filament::SwapChain* swapChain = engine->createSwapChain((window->src.window));
...
//    tcm.setTransform(tcm.getInstance(renderable), filament::math::mat4f::rotation(180.0f * HMM_DegToRad, filament::math::float3{ 0, 1, 0 }));
    ...
    ImGui_ImplRgfw_InitForOther(window, true);
    ...
    {
        RGFW_window_checkEvents(window, RGFW_NO_WAIT);

        // float now = (double)(RGFW_getTimeNS() / (1e9)) - startTime;
        ...
        const double now = static_cast<double>(std::chrono::duration_cast<std::chrono::milliseconds>(time - startTime).count()) / 1000.0f;
