...
//            .normalized(filament::VertexAttribute::COLOR)
    ...
    filament::SkinningBuffer* skinningBuffer = filament::SkinningBuffer::Builder()
            .boneCount(ozz.joint_matrices_fmath.size())
            .initialize()
            .build(*engine);
    skinningBuffer->setBones(*engine, ozz.joint_matrices_fmath.data(), ozz.joint_matrices_fmath.size(), 0);
...
//            .skinning(skinningBuffer, ozz.joint_matrices_fmath.size(), 0)
            .build(*engine, renderable);
    scene->addEntity(renderable);
    auto& tcm = engine->getTransformManager();
    tcm.setTransform(tcm.getInstance(renderable), filament::math::mat4f::rotation(0.0f, filament::math::float3{ 0, 0, 1 }));
...
//            memcpy(ozzJointBuffer.getDataAddress(), ozz.joint_matrices.data(), sizeof(ozz.joint_matrices[0]) * ozz.joint_matrices.size());
//            skinningBuffer->setBones(*engine, ozz.joint_matrices_fmath.data(), ozz.joint_matrices_fmath.size(), 0);
        }
